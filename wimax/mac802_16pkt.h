...
u_char type_mesh       :
    1;  // mesh subheader: 1=present, 0=absent
u_char type_arqfb      :
    1;  // ARQ Feedback payload: 1=present, 0=absent
u_char type_ext        :
    1;  // Extended type
u_char type_frag       :
    1;  // Fragmentation subheader: 1=present, 0=absent
u_char type_pck        :
    1;  // Packing subheader: 1=present, 0=absent
u_char type_fbgm       :
    1;  // DL: FAST-FEEDBACK allocation subheader, UL: Grant Management subheader
...
u_char ec :
    1;
u_char type :
    3;      //use this MSB => 00:BW_REQ, 01:CDMA-BW_REQ, 11:CDMA-INIT
    u_char top;           //use 8 bits so max_opportunity is 2^8 = 256
u_int16_t br :
    11;    //use this "br" to indicate subcriber id so max => 2^11 = 2048
    u_char code;
    u_int16_t cid;
};
...
u_char      ack_type        :
    2;     // Type of ACK - selective =0 , cumulative = 1, cumulative with selective ACK = 2, reserved = 3
u_int32_t   fsn             :
    11;    // sequence number
};
...
char fc :
    2; 		//00:no fragmentation, 01:last, 10: first, 11: middle
char fsn :
    3; 	//fragmentation number
u_int16_t sn:
    11; 	//If this is an ARQ-enabled connection, this is the BSN, otherwise it is the FSN
...
u_char subchannel_offset:
    6;
u_char boosting:
    3;
u_char num_of_symbols:
    7;
...
u_int16_t num_of_subchannels:
    14;

u_char repition_coding_indicator:
    2;
// rpi
};
...
u_char scanning_type:
    3; //0: scanning without association
...
u_char net_assisted_ho_supported:
    1;
u_char ho_id_included_indicator:
    1;
u_char ho_autho_policy_indicator:
    1;
...
u_char duration:
    3;
u_char mimo_permutation_feedback:
    2;
};
...
u_char resource_retain_flag:
    1;
    mac802_16_mob_bsho_req_mode_000 n_rec[];
    //if mode == 0b001
u_char tmp_bsid:
    3;
u_char ak_change_indicator:
    1;
    u_char n_cids;
...
u_char service_level_prediction:
    3;
u_char arrival_time_diff_ind:
    1;
    //next if arrival_time_diff_ind==1
u_char arrival_time_diff:
    4;
};
