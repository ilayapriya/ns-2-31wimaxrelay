...
*/
#include <cstdlib>
#include "mac802_16RS.h"
#include "scheduling/wimaxscheduler.h"
#include "scheduling/rsscheduler.h"
...
*/
static class Mac802_16RSClass:public TclClass
{
public:
    Mac802_16RSClass ():TclClass ("Mac/802_16/RS")
    {
    }
    TclObject *create (int, const char *const *)
    {
        return (new Mac802_16RS ());
...
*/
Mac802_16RS::Mac802_16RS ():Mac802_16 (), cl_head_ (0), cl_tail_ (0),
        ctrlagent_ (0),ul(0)
{
    type_ = STA_RS;		//type of MAC. In this case it is for RS

    //Default configuration
    addClassifier (new DestClassifier ());
    scheduler_ = new RSScheduler ();
    ...
    //scanning status
    scan_info_ =
        (struct scanning_structure *) malloc (sizeof (struct scanning_structure));
...
*/
int
Mac802_16RS::command (int argc, const char *const *argv)
{
    if (argc == 3)
    {
...
*/
void
Mac802_16RS::init_default_connections ()
{
    Connection *con;

    //create initial ranging and padding connection
    con = new Connection (CONN_INIT_RANGING);
    connectionManager_->add_connection (con, OUT_CONNECTION);	//uplinkdebug2("12\n");
    con = new Connection (CONN_INIT_RANGING);
    connectionManager_->add_connection (con, IN_CONNECTION);	//downlinkdebug2("12\n");
...
Mac802_16RS::init ()
{
    ...
    */
    double stime =
        getFrameDuration () + Random::uniform (0, getFrameDuration ());
    dl_timer_->sched (stime);
    //TODO revised : bu olmayınca çalışmıyor: following code line commented
    ul_timer_->sched(stime);
...
*/
void
Mac802_16RS::setMacState (Mac802_16State state)
{
    state_ = state;
}
...
*/
void
Mac802_16RS::restore_state (state_info * backup_state)
{
...
*/
void
Mac802_16RS::expire (timer_id id)
{
    switch (id)
    {
    ...
    case WimaxT2TimerID:
        debug
        ("At %f in Mac %d, lost synchronization (RNG) so remove cdma initial ranging request\n",
         NOW, addr ());
    ...
    case WimaxT3TimerID:
        debug
        ("At %f in Mac %d, no response from BS so remove cdma initial ranging request\n",
         NOW, addr ());
    ...
    case WimaxT44TimerID:
        debug
        ("At %f in Mac %d, did not receive MOB_SCN-RSP (nb_retry=%d/%d)\n",
    ...
    default:
        debug ("Warning: unknown timer expired in Mac802_16RS\n");
    }
}
...
Mac802_16RS::sendDown (Packet * p)
{
    ...
    {
        debug
        ("At %f in Mac %d drop packet because no classification were found\n",
         NOW, index_);
        drop (p, "CID");
        //Packet::free (p);
    }
    else
    {
        ...
        Connection *connection =
            connectionManager_->get_connection (cid, OUT_CONNECTION);
        if (connection == NULL)
        {
            debug
            ("Warning: At %f in Mac %d connection with cid = %d does not exist. Please check classifiers\n",
             NOW, index_, cid);
            //Packet::free (p);
        ...
        else
        {
            ...
            else if (connection->getArqStatus ()
                     && (connection->getArqStatus ()->isArqEnabled () == 1))
            {
                int data_packet_size = HDR_CMN (p)->size ();
                int num_arq_blocks =
                    (int) ceil ((double) data_packet_size / arq_block_size_);

                debug2
                ("data_packet_size=%d\t num_arq_blocks=%d\t qlength=%d max_qlength=%d\n",
                 data_packet_size, num_arq_blocks, connection->queueLength (),
                 macmib_.queue_length);

                if ((connection->queueLength () + num_arq_blocks) >=
                        macmib_.queue_length)
                {
                    update_watch (&loss_watch_, 1);
                    need_drop = true;
            ...
            if (!need_drop)
            {
                if (connection->getArqStatus () == NULL)
                {
                    ...
                    debug2
                    ("At %f (RS) in Mac %d, SENDDOWN, Enqueue packet to cid :%d queue size :%d (max :%d)\n",
                     NOW, index_, cid, connection->queueLength (),
                     macmib_.queue_length);
                }
                else if ((connection->getArqStatus () != NULL)
                         && (connection->getArqStatus ()->isArqEnabled () == 1))
                {
                    ...
                    {
                        if (i == 1)
                            connection->getArqStatus ()->arqSend (p, connection,
                                                                  FRAG_FIRST);
                        else
                            connection->getArqStatus ()->arqSend (p, connection,
                                                                  FRAG_CONT);
                    ...
                    HDR_CMN (p)->size () =
                        (packet_size - (number_of_packs * arq_block_size_));
                    if (number_of_packs)
                    {
                        connection->getArqStatus ()->arqSend (p, connection,
                                                              FRAG_LAST);
                    ...
                    connection->enqueue (p);
                    debug2
                    ("ARQ RS Dividing Packet into Blocks Packet Size=%d, Number of packets = %d, ARQ_BLOCK_SIZE = %d,Last Packet Size= %d\n",
                     packet_size, number_of_packs + 1, arq_block_size_,
                     HDR_CMN (p)->size ());
                }
            }
        }
...
*/
void
Mac802_16RS::transmit (Packet * p)
{
    struct hdr_cmn *ch = HDR_CMN (p);
    int if_cdma_packet = 0;

    debug
    ("At %f (RS) in Mac %d, sending packet, TRANSMIT, (type :%s, size :%d, txtime :%.9f)\n",
     NOW, index_, packet_info.name (ch->ptype ()), ch->size (),
     ch->txtime ());
    if (ch->ptype () == PT_MAC)
    {
...
Mac802_16RS::sendUp (Packet * p)
{
...
#ifdef DEBUG_WIMAX
    debug10
    ("At %f (RS) in Mac %d, SENDUP, receive first bit..over at :%f(txtime :%f) (type :%s, size :%d), cid :%d\n",
     NOW, index_, NOW + ch->txtime (), ch->txtime (),
    ...
    if ((wimaxHdr->phy_info.OFDMSymbol_offset == 0 && wimaxHdr->phy_info.num_OFDMSymbol == 0) || wimaxHdr->header.cid == BROADCAST_CID)  	// this kind of packets are treated diff(OFDM types) basically it a bw req packet.
    {
        if (con == NULL)
        {
            ...
            Packet::free (p);
            debug2 (" packet getting dropped header.ht :%d",
                    HDR_MAC802_16 (p)->header.ht);
            //pktRx_=NULL;
            return;
        }
        ...
        if (contPktRxing_ == TRUE)
        {
            ...
            if (head_pkt_ != NULL)
            {
                ...
                if (head_pkt_->p->txinfo_.RxPr / p->txinfo_.RxPr >=
                        p->txinfo_.CPThresh)
                {
    ...
    else
    {
        ...
        if (con == NULL)
        {
            //This packet is not for us
            debug2
            ("At %f in Mac %d Connection null -- packet getting dropped at RS not for us \n",
             NOW, index_);
    ...
    */
    ...
    {
        debug10
        (" RS, SENDUP, received by pass cdma_bw_req code :%d, top :%d, ssid :%d\n",
         req_tmp->code, req_tmp->top, req_tmp->br);
        addPacket (p);
        return;
    }
    ...
    {
        debug10
        (" RS, SENDUP, received by pass cdma_init_req code :%d, top :%d, ssid :%d\n",
         req_tmp->code, req_tmp->top, req_tmp->br);
        addPacket (p);
        return;
    }
        ...
        if (HDR_MAC802_16 (pktRx_)->header.ht == 0)
        {
            if ((mac802_16_dl_map_frame *) pktRx_->accessdata () != NULL)
                debug10 (" generic mngt :%d\n",
                         ((mac802_16_dl_map_frame *) pktRx_->accessdata ())->
                         type);
        }
        else
        {
            ...
            {
                debug10
                ("\t Impossible (PANIC) cdma_bw_req code :%d, top :%d\n",
            ...
            {
                debug10
                ("\t Impossible (PANIC) cdma_init_req code :%d, top :%d\n",
                 req_tmp->code, req_tmp->top);
                flag_cdma_packet = 1;
            }
            else
...
// calculate the total subcarriers needed
        if (wimaxHdr->phy_info.num_OFDMSymbol != 0)
        {
            ...
            if (wimaxHdr->phy_info.num_OFDMSymbol % num_symbol_per_slot == 0)  	// chk this condition , chk whether broacastcid reqd ir not.
            {
                if (wimaxHdr->phy_info.num_OFDMSymbol > num_symbol_per_slot)
                {
                    ...
                    for (int i =
                            ...
                            i <
                            wimaxHdr->phy_info.OFDMSymbol_offset +
                            wimaxHdr->phy_info.num_OFDMSymbol; i++)
            ...
            else
            {
                ...
                {
                    for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                            i < (wimaxHdr->phy_info.OFDMSymbol_offset + 1); i++)
                        //   for(int j = (wimaxHdr->phy_info.subchannel_offset)*num_data_subcarrier ; j<num_subchannel*num_data_subcarrier ; j++ )
                    {
                        total_subcarriers +=
                            num_subchannel * num_data_subcarrier -
                    ...
                    for (int i = wimaxHdr->phy_info.OFDMSymbol_offset + 1;
                            i <
                            (wimaxHdr->phy_info.OFDMSymbol_offset) +
                            (wimaxHdr->phy_info.num_OFDMSymbol - 1); i++)
                        //for(int j = 0 ; j<num_subchannel*num_data_subcarrier ; j++ )
                    {
                        total_subcarriers +=
                            num_subchannel * num_data_subcarrier;
            ...
            if (wimaxHdr->phy_info.num_OFDMSymbol > num_symbol_per_slot)
            {
                ...
                for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                    ...
                    for (int j =
                                (wimaxHdr->phy_info.subchannel_offset) *
                                num_data_subcarrier;
                            j < num_subchannel * num_data_subcarrier; j++)
                    {
                        interferencepower[n++] =
                            intpower_[i][j] +
                            BASE_POWER /*+ pktRx_->txinfo_.RxIntPr[i][j] */ ;
                ...
                for (int i =
                            wimaxHdr->phy_info.OFDMSymbol_offset + num_symbol_per_slot;
                        i <
                        (wimaxHdr->phy_info.OFDMSymbol_offset) +
                        (wimaxHdr->phy_info.num_OFDMSymbol - num_symbol_per_slot);
                        i++)
                    for (int j = 0; j < num_subchannel * num_data_subcarrier; j++)
                    {
                        interferencepower[n++] =
                ...
                for (int i =
                    ...
                    for (int j = 0;
                            ...
                            ((wimaxHdr->phy_info.num_subchannels -
                              (num_subchannel -
                               (wimaxHdr->phy_info.subchannel_offset))) %
                             num_subchannel) * num_data_subcarrier; j++)
                    {
                        interferencepower[n++] =
                            intpower_[i][j] +
                            BASE_POWER /*+ pktRx_->txinfo_.RxIntPr[i][j] */ ;
...
// BLER calculation
        ...
        int num_of_slots, num_of_complete_block, max_block_size,
        last_block_size, index, num_subcarrier_block;

        double BLER = 0.0, beta = 0.0, eesm_sum = 0.0, rand1 = 0.0;

        num_of_slots =
            (int) ceil ((double) ch->size () /
                        (double) phy->getSlotCapacity (wimaxHdr->phy_info.
                                                       modulation_, DL_));
        ...
        debug2
        (" num of complete blocks = %d , ITU_PDP = %d, num of slots = %d, max block size = %d, last block size = %d \n",
         num_of_complete_block, ITU_PDP, num_of_slots, max_block_size,
         last_block_size);
        ...
        if (num_of_complete_block == 0)
        {
            ...
            index =
                phy->getMCSIndex (wimaxHdr->phy_info.modulation_,
                                  last_block_size);
            ...
            num_subcarrier_block =
                num_symbol_per_slot * getPhy ()->getNumSubcarrier (DL_) *
                last_block_size;
        ...
        else
        {
            ...
            index =
                phy->getMCSIndex (wimaxHdr->phy_info.modulation_, max_block_size);
...
// deleting dynamic allocations  //removed for testing without Rxinpwr in packet header
...

        {
            ...
            debug2
            ("error in the packet, drop this packet. the Mac does not process\n");
            Packet::free (pktRx_);
            pkt_error = FALSE;
            //update drop stat
            update_watch (&loss_watch_, 1);
...
//update rx time of last packet received
    ...
    if (peer_)
    {
        ...
        {
            printf
            ("ERROR: RXpr = 0, please double check; pktRx_->txinfo_.RxPr :%g\n",
             pktRx_->txinfo_.RxPr);
            exit (1);
        }
...
// New function for reassembly will be implemented for ARQ, Fragmentation and Packing
    if (con->getArqStatus () != NULL
            && con->getArqStatus ()->isArqEnabled () == 1
...
//process reassembly
    if (wimaxHdr->header.type_frag)
    {
        ...
        debug2 (" Frag type = %d %d\n", wimaxHdr->frag_subheader.fc,
                wimaxHdr->frag_subheader.fc & 0x3);
        ...
        case FRAG_FIRST:
            ...
            con->updateFragmentation (FRAG_FIRST, 0,
                                      ch->size () - (HDR_MAC802_16_SIZE +
                                                     HDR_MAC802_16_FRAGSUB_SIZE));
            break;
        case FRAG_CONT:
            if ((con->getFragmentationStatus () != FRAG_FIRST
                    && con->getFragmentationStatus () != FRAG_CONT)
                    || ((wimaxHdr->frag_subheader.fsn & 0x7) !=
                        (con->getFragmentNumber () + 1) % 8))
            {
                frag_error = true;
                con->updateFragmentation (FRAG_NOFRAG, 0, 0);	//reset
            }
            else
            {
                ...
                con->updateFragmentation (FRAG_CONT,
                                          wimaxHdr->frag_subheader.fsn & 0x7,
                                          con->getFragmentBytes () +
                                          ch->size () - (HDR_MAC802_16_SIZE +
                                                         HDR_MAC802_16_FRAGSUB_SIZE));
            }
            break;
        case FRAG_LAST:
                        ...
                        (con->getFragmentNumber () + 1) % 8))
            {
                ...
                ch->size () +=
                    con->getFragmentBytes () - HDR_MAC802_16_FRAGSUB_SIZE;
                drop_pkt = false;
            }
            else
            {
...
*/
void
Mac802_16RS::process_mac_pdu_witharqfragpack (Connection * con, Packet * pkt)
{
    ...
    debug2 ("ARQ RS: Entering the process_mac_pdu_witharqfragpack function \n");
    ...
    if (wimaxHdr_pdu->header.type_arqfb == 1)
    {
        ...
        {
            Connection *connection =
                this->getCManager ()->get_connection (wimaxHdr_pdu->arq_ie[i].cid,
                                                      true);
            if (connection)
            {
                if (connection->getArqStatus ()
                ...
                {
                    debug2
                    ("ARQ : RS Feedback in Data Payload Received: Has a feedback: Value of i:%d , Value of number of acks:%d \n",
                     i, wimaxHdr_pdu->num_of_acks);
                    connection->getArqStatus ()->arqRecvFeedback (mac_pdu, i,
                            connection);
                }
            }
        }
        ch_pdu->size () -=
            (wimaxHdr_pdu->num_of_acks * HDR_MAC802_16_ARQFEEDBK_SIZE);
    }
    ...
    {
        debug2
        ("ARQ: RS process_mac_pdu_witharqfragpack %f Drop data loss-- packet size [%d] %f %f\n",
    ...
    {
        debug2
        ("ARQ: RS process_mac_pdu_witharqfragpack %f No drop data loss %f %f\n",
         NOW, tmp, data_loss_);
    }
    ...
    debug2 ("ARQ RS: before update, the ch_pdu size: %d \n", ch_pdu->size ());
    while (p_temp)
    {
        // Update the Mac PDU Size
        debug2 ("ARQ:RS p-previous :%p p-temp : %p \n", p_previous, p_temp);
        debug2 ("ARQ : RS type of ARQ to update size:  %d, %d\n ",
                HDR_MAC802_16 (p_temp)->pack_subheader.fc,
                (((HDR_MAC802_16 (p_temp)->pack_subheader.sn) -
                  (HDR_MAC802_16 (p_previous)->pack_subheader.sn)) & 0x7FF));

        if ((HDR_MAC802_16 (p_temp)->pack_subheader.fc == FRAG_FIRST)
                ||
                ((((HDR_MAC802_16 (p_temp)->pack_subheader.sn) -
                   (HDR_MAC802_16 (p_previous)->pack_subheader.sn)) & 0x7FF) != 1)
                || (HDR_MAC802_16 (p_temp)->pack_subheader.fc == FRAG_NOFRAG))
            ch_pdu->size () -=
                HDR_CMN (p_temp)->size () + HDR_MAC802_16_FRAGSUB_SIZE;
        else
    ...
    debug2 ("ARQ RS: Finally the ch_pdu size: %d \n", ch_pdu->size ());
            ...
            && con->getArqStatus ()->isArqEnabled () == 1)
    {
        ...
        debug2 ("Receive: ARQ RS :ORDER is:%d\n", isInOrder);
        if (isInOrder == 0)  	/*In middle part */
        {
            ...
            debug2
            ("Receive: ARQ RS : Out of order packet, buffering in retransmission queue\n");
        ...
        else if (isInOrder == 1)  	/*in order */
        {
            ...
            debug2
            ("Receive: ARQ RS : In order packet, buffering in arq transmission queue\n");
            con->getArqStatus ()->arq_trans_queue_->enque (mac_pdu->copy ());
        }
        else  		/*outside of ARQ window. */
        {
            debug2
            ("Receive: ARQ RS : Not buffering in arq trans/retrans queue\n");
        }
    }
    ...
    while (p_current)
    {
                ...
                && con->getArqStatus ()->isArqEnabled () == 1)
        {
            ...
            debug2 ("Receive: ARQ RS cid=%d:ORDER is:%d\n", con->get_cid (),
                    isInOrder);
            if (isInOrder == 0)
            {
                // packet is out of order, so queue it in retransmission queue at receiver end since it is not being used
                debug2
                ("Receive: ARQ RS cid=%d: Out of order packet, buffering in retransmission queue\n",
                 con->get_cid ());
                con->getArqStatus ()->arq_retrans_queue_->enque (p_current->
                        copy ());
            }
            else if (isInOrder == 1)
            {
                // packet is in order, so queue it in transmission queue at receiver end since it is not being used
                debug2
                ("Receive: ARQ RS cid=%d: In order packet, buffering in arq transmission queue\n",
                 con->get_cid ());
                con->getArqStatus ()->arq_trans_queue_->enque (p_current->
                        copy ());
            }
            else
            {
...
secondary:
        ...
        {
            debug2
            ("ARQ RS cid=%d: Retransmission queue is empty- so nothing to fit or packets are there but of no use\n",
             con->get_cid ());
            break;
        }
                ...
                && con->getArqStatus ()->isArqEnabled () == 1)
        {
            ...
            debug2 ("ARQ RS cid=%d: isInOrder=%d\n", con->get_cid (),
                    isInOrder);
            if (isInOrder == 2)  	/*Just outside of ARQ window. We simply remove it since it is of no use. */
            {
                ...
                debug2 ("ARQ RS remove out-order ARQ blocks.\n");
                Packet::free (p);
                goto secondary;
            }
            else if (isInOrder == 0)
            ...
            else  		/*In order. */
            {
                ...
                while (con->getArqStatus ()->arq_retrans_queue_)
                {
                    ...
                    {
                        debug2
                        ("ARQ RS cid=%d: Retransmission2 queue is empty- so nothing to fit or packets are there but of no use\n",
                         con->get_cid ());
                        break;
                    }
                    ...
                    {
                        isInOrder = 1;
                        con->getArqStatus ()->arqReceiveBufferTransfer (p, con,
                                &isInOrder);
                        debug2 ("ARQ RS cid=%d: isInOrder2=%d\n",
                                con->get_cid (), isInOrder);
                        if (isInOrder == 2)
                        {
                            con->getArqStatus ()->arq_retrans_queue_->
                            remove (p);
                            Packet::free (p);
                            goto primary;
                        }
                        ...
                        {
                            con->getArqStatus ()->arq_trans_queue_->enque (p->
                                    copy
                                    ());
                            con->getArqStatus ()->arq_retrans_queue_->
                            remove (p);
    ...
    while (1)
    {
        ...
        if (!p)
        {
            debug2 ("ARQ RS: Transmission Queue is Empty \n ");
            break;
        }
        wimaxHdr = HDR_MAC802_16 (p);
        ch = HDR_CMN (p);
        ...
        if (wimaxHdr->pack_subheader.fc == FRAG_FIRST)
        {
            ...
            debug2
            ("ARQ RS: This is the first packet in transmission queue CID:[%d] and FRAG_FIRST Fsn: %d Size :%d\n",
             wimaxHdr->header.cid, wimaxHdr_sdu->pack_subheader.sn,
             ch_sdu->size ());
            ...
            while (1)
            {
                ...
                {
                    debug2
                    ("ARQ RS: Except for the first Block, all other ARQ Blocks for this MAC SDU are not in ARQ transmission queue. \n");
                    break;
                }
                wimaxHdr = HDR_MAC802_16 (p);
                ch = HDR_CMN (p);
                ...
                if ((wimaxHdr->pack_subheader.fc == FRAG_CONT)
                        && (((wimaxHdr->pack_subheader.sn - seqno) & 0x7FF) == 1))
                {
                    debug2
                    ("ARQ:RS Generating the MAC SDU CID %d Current SN: %d Previous SN: %d Frag Type: %d Size: %d, %d \t Trans-Q length is [%d] Re-trans-Q length is %d \n ",
                     con->get_cid (), wimaxHdr->pack_subheader.sn, seqno,
                     wimaxHdr->pack_subheader.fc, ch->size (),
                     ch_sdu->size (),
                     con->getArqStatus ()->arq_trans_queue_->length (),
                     con->getArqStatus ()->arq_retrans_queue_->length ());
                    ch_sdu->size () += ch->size ();
                    seqno++;
                }
                else if (wimaxHdr->pack_subheader.fc == FRAG_LAST
                         && (((wimaxHdr->pack_subheader.sn - seqno) & 0x7FF) ==
                             1))
                {
                    debug2
                    ("ARQ: RS Generating the MAC SDU CID %d Current SN: %d Previous SN: %d Frag Type: %d Size: %d, %d \t Trans-Q length is [%d] Re-trans-Q length is %d \n ",
                ...
                {
                    debug2
                    ("ARQ RS:This is a unsual case FSN: %d  and Frag_Type: %d \n",
                     wimaxHdr->pack_subheader.sn,
                     wimaxHdr->pack_subheader.fc);
                    break;
                }
            }
            ...
            if (mac_sdu_gen == true)
            {
                ...
                debug2 ("ARQ RS: Created the Mac SDU SIZE: %d\n ",
                        ch_sdu->size ());
                //update_throughput (&rx_data_watch_, 8*pdu_size);
                //update_throughput (&rx_traffic_watch_, 8*pdu_size);
                debug2
                ("send to upper layer, Mac SDU size [%d]  time[%f] for MAC [%d] Last block FSN is [%d]\n",
                ...
                do
                {
                    ...
                    Packet::free (p_temp);
                    debug2
                    ("ARQ RS cid=%d: To keep a count on how many times Dequeue function is called \n",
                     con->get_cid ());
                    ...
                    ch = HDR_CMN (p);
                    debug2
                    ("ARQ RS: Next Packet to Dequeue FSN: %d FRAG_STATUS: %d \n",
                     wimaxHdr->pack_subheader.sn,
                     wimaxHdr->pack_subheader.fc);
                }
                while (wimaxHdr->pack_subheader.fc != FRAG_FIRST);
                debug2
                ("ARQ:RS Generating the MAC SDU CID %d Trans-Q length is [%d] Re-trans-Q length is [%d] \n ",
                 con->get_cid (),
                 con->getArqStatus ()->arq_trans_queue_->length (),
                 con->getArqStatus ()->arq_retrans_queue_->length ());
            ...
            {
                debug2
                ("ARQ RS: Either no packets in trans queue or all the mac sdu that could be generated are sent above \n");
                goto end;
            }
        }
        else if (wimaxHdr->pack_subheader.fc == FRAG_NOFRAG)  	// This will be the only part of the MAC SDU
        {
            ...
            seqno = wimaxHdr_sdu->pack_subheader.sn;
            debug2
            ("ARQ RS cid=%d: This is the only fragment  of MAC SDU FRAG_NOFRAG Fsn: %d Size :%d\n",
             con->get_cid (), wimaxHdr_sdu->pack_subheader.sn,
             ch_sdu->size ());
            debug2 ("ARQ RS cid=%d: Created the Mac SDU SIZE: %d\n ",
                    con->get_cid (), ch_sdu->size ());

            //update_throughput (&rx_data_watch_, 8*pdu_size);
            //update_throughput (&rx_traffic_watch_, 8*pdu_size);
            ch_sdu->direction () == hdr_cmn::UP; //TODO revised no problem: SS de bu kısım yok geçi saçma bir işlem
            uptarget_->recv (mac_sdu, (Handler *) 0);
            mac_sdu_gen = false;
        ...
        {
            debug2
            ("ARQ RS  cid=%d: Something really wrong with the implementation\n",
             con->get_cid ());
            abort ();
        }
    }
...
*/
void
Mac802_16RS::process_dl_map (mac802_16_dl_map_frame * frame)
{
    assert (frame);
                ...
                || scan_info_->substate == SCANNING)
        {
            if (scan_info_->substate == SCANNING)
            {
                if (scan_info_->nbr == NULL
                        || scan_info_->nbr->getID () != frame->bsid)
                {
            ...
            if (dcd != NULL)
            {
                ...
                for (int i = 0;
                        !error
                        && i <
                        getMap ()->getDlSubframe ()->getPdu ()->getNbBurst (); i++)
                {
                    int diuc =
                        getMap ()->getDlSubframe ()->getPdu ()->getBurst (i)->
                        getIUC ();
    ...
    else
    {
                ...
                && getMacState () != MAC802_16_UL_PARAM)
        {
            ...
            stime +=
                getMap ()->getDlSubframe ()->getPdu ()->getBurst (1)->
                getStarttime () * getPhy ()->getSymbolTime ();
            ...
            dl_timer_->resched (getMap ()->getStarttime () +
                                getFrameDuration () - NOW);
        }
    }
}
...
*/
void
Mac802_16RS::process_dcd (mac802_16_dcd_frame * frame)
{
    if (getMacState () == MAC802_16_WAIT_DL_SYNCH)
    {
    ...
    if (getMacState () == MAC802_16_WAIT_DL_SYNCH_DCD)
    {
        ...
        {
            getMap ()->setStarttime (getMap ()->getStarttime () +
                                     getFrameDuration ());
        }
        ...
        if (scan_info_->substate == SCANNING)
        {
            mac802_16_dcd_frame *tmp =
                (mac802_16_dcd_frame *) malloc (sizeof (mac802_16_dcd_frame));
...
*/
void
Mac802_16RS::process_ucd (mac802_16_ucd_frame * frame)
{
    if (getMacState () == MAC802_16_WAIT_DL_SYNCH
    ...
    if (getMacState () == MAC802_16_UL_PARAM)
    {
        ...
        if (scan_info_->substate == SCANNING)
        {
            mac802_16_ucd_frame *tmp =
                (mac802_16_ucd_frame *) malloc (sizeof (mac802_16_ucd_frame));
        ...
        //change state
        debug10
        (" received ucd and prepare to do init_ranging set state to MAC802_16_RANGING\n");
...
*/
void
Mac802_16RS::process_ul_map (mac802_16_ul_map_frame * frame)
{
    if (getMacState () == MAC802_16_WAIT_DL_SYNCH
            || getMacState () == MAC802_16_WAIT_DL_SYNCH_DCD)
    {
        //discard the packet
        debug
        ("At %f in %d, drop UL_MAP because not in right state (state=%d)\n",
    ...
    if (getMacState () == MAC802_16_UL_PARAM)
    {
                ...
                || scan_info_->substate == SCANNING)
        {
            ...
            if (ucd != NULL)
            {
                ...
                bool found;
                for (int i = 0;
                        !error && i < tmpMap->getUlSubframe ()->getNbPdu (); i++)
                {
                    UlBurst *b =
                        (UlBurst *) tmpMap->getUlSubframe ()->getPhyPdu (i)->
                        getBurst (0);
                    int uiuc = b->getIUC ();
                    if (uiuc == UIUC_END_OF_MAP)
                        continue;
                    if (uiuc == UIUC_EXT_UIUC
    ...
    if (scan_info_->substate == SCANNING)
    {
        ...
        {
            if (scan_info_->rsp->rec_bs_full[i].recommended_bs_id ==
                    scan_info_->nbr->getID ())
            {
...
*/
void
Mac802_16RS::process_ranging_rsp (mac802_16_rng_rsp_frame * frame)
{
    ...
    {
    case RNG_SUCCESS:
        debug2("RS ranging success\n");
        debug
        ("Ranging response (remove cdma intial ranging request) : status = Success.Basic :%d, Primary :%d\n",
        ...
        if (scan_info_->substate == SCANNING)
        {
            ...
            mac802_16_rng_rsp_frame *tmp =
                (mac802_16_rng_rsp_frame *)
                malloc (sizeof (mac802_16_rng_rsp_frame));
        ...
        else
        {
            if (basic != NULL)
            {
                ...
                {
                    getCManager ()->remove_connection (peer->
                                                       getSecondary
                                                       (IN_CONNECTION));
                    getCManager ()->remove_connection (peer->
                                                       getSecondary
            ...
            //      upbasic->initCDMA_RSID();
            primary = new Connection (CONN_PRIMARY, frame->primary_cid);
            Connection *upprimary =
                new Connection (CONN_PRIMARY, frame->primary_cid);
            //      upprimary->setBW_REQ_QUEUE(0);
            //      upprimary->initCDMA_RSID();

            //a RS should only have one peer, the BS
    ...
    case RNG_CONTINUE:
    case RNG_RERANGE:
        debug2("RS ranging Different  response came\n");
        break;
    default:
...
*/
void
Mac802_16RS::init_ranging ()
{
    debug2("rs ranging init\n");
    ...
    if (scan_info_->substate == HANDOVER
            && scan_info_->nbr->getRangingRsp () != NULL)
    {
    ...
    for (PhyPdu * p = getMap ()->getUlSubframe ()->getFirstPdu (); p;
            p = p->next_entry ())
    {
                ...
                b->getFastRangingMacAddr () == addr ())
        {
            ...
            mac802_16_rng_req_frame *frame =
                (mac802_16_rng_req_frame *) p->accessdata ();
    ...
    for (PhyPdu * pdu = ulsubframe->getFirstPdu (); pdu;
            pdu = pdu->next_entry ())
    {
        if (pdu->getBurst (0)->getIUC () == UIUC_INITIAL_RANGING)
        {
            debug10
            ("At %f (RS) Mac %d found ranging opportunity => prepare to create MAC_RNG_REQ (cdma_init_ranging proccess)\n",
             NOW, addr ());
            /*
            //Old code
            Packet *p= getPacket();
            ...
            */
            ...
            if (getMap ()->getUlSubframe ()->getRanging ()->
                    getRequest_mac (addr ()) != NULL)
            {
                debug2
                ("Create CDMA-INIT-REQ, RS addr :%d -- already pending requests => return\n",
                 addr ());
                continue;
            }
            else
            {
                debug2 ("Create CDMA-INIT-REQ, RS addr :%d\n", addr ());
            }
            ...
            wimaxHdr->phy_info.subchannel_offset =
                sub_channel_off * fix_six_subchannel;
            ...
            int code_range =
                macmib_.cdma_code_init_start - macmib_.cdma_code_init_stop + 1;
            int c_rand = rand () % code_range;
            u_char header_code =
                (u_char) macmib_.cdma_code_init_start + (u_char) c_rand;
            ...
            cdma_req_header_t *header =
                (cdma_req_header_t *) & (HDR_MAC802_16 (p)->header);
            header->ht = 1;
            header->ec = 1;
            header->type = 0x2;
            ...
            debug10
            (" RSscheduler enqueued cdma_initial ranging request for cid :%d, q-len :%d, (nbPacket :%d), code :%d, top :%d, ss_id :%d, backoff :%d, window :%d, nbretry :%d, flagtransmit :%d, flagnowtransmit :%d\n",
             INITIAL_RANGING_CID, 0, 0, header_code, header_top, header->br,
             result, s_window, s_nbretry, s_flagtransmit, s_flagnowtransmit);

            debug10
            (" symbol_offset :%d, symbol :%d, subchannel_offset :%d, subchannel :%d, channel_index :%d, direction :%d, txtime :%e\n",
             wimaxHdr->phy_info.OFDMSymbol_offset,
             wimaxHdr->phy_info.num_OFDMSymbol,
             wimaxHdr->phy_info.subchannel_offset,
             wimaxHdr->phy_info.num_subchannels,
             wimaxHdr->phy_info.channel_index, wimaxHdr->phy_info.direction,
             ch->txtime ());
...
*/
void
Mac802_16RS::process_reg_rsp (mac802_16_reg_rsp_frame * frame)
{
    //check the destination
    PeerNode *peer = getPeerNode_head ();
    ...
    if (frame->response == 0)
    {
        ...
        if (!secondary)
        {
            Connection *secondary =
                new Connection (CONN_SECONDARY, frame->sec_mngmt_cid);
            //      secondary->setBW_REQ_QUEUE(0);
            //      secondary->initCDMA_SSID();
            Connection *upsecondary =
                new Connection (CONN_SECONDARY, frame->sec_mngmt_cid);
        ...
        */
        ...
        if (scan_info_->substate == HANDOVER)
        {
            ...
            send_link_handover_complete (addr (), scan_info_->serving_bsid,
                                         peer->getAddr ());
...
Mac802_16RS::send_scan_request ()
{
    ...
    {
        debug
        ("At %f in Mac %d scan request invalid because MAC is disconnected\n",
         NOW, addr ());
        return;
    }
...
*/
void
Mac802_16RS::process_scan_rsp (mac802_16_mob_scn_rsp_frame * frame)
{
    //PeerNode *peer = getPeerNode_head();
    if (!t44timer_->busy ())
    {
    ...
    if (frame->scan_duration != 0)
    {
        ...
        debug ("At %f in Mac %d, scanning accepted (dur=%d it=%d)\n", NOW,
               addr (), frame->scan_duration, frame->scan_iteration);
        ...
        scan_info_->rsp =
            ...
            malloc (sizeof (struct mac802_16_mob_scn_rsp_frame));
        memcpy (scan_info_->rsp, frame,
                sizeof (struct mac802_16_mob_scn_rsp_frame));
        ...
        debug ("\tstart scan in %d frames (%f)\n", frame->start_frame,
               NOW + frame->start_frame * getFrameDuration ());
        for (int i = 0; i < scan_info_->rsp->n_recommended_bs_full; i++)
        {
            if (scan_info_->rsp->rec_bs_full[i].scanning_type == SCAN_ASSOC_LVL1
                    || scan_info_->rsp->rec_bs_full[i].scanning_type ==
                    SCAN_ASSOC_LVL2)
            {
                debug2 ("Creating timer for bs=%d at time %f\n",
                        ...
                        NOW +
                        getFrameDuration () *
                        scan_info_->rsp->rec_bs_full[i].rdv_time);
                assert (nbr_db_->
                        getNeighbor (scan_info_->rsp->rec_bs_full[i].
                                     recommended_bs_id));
                //get the channel
                int ch =
                    getChannel (nbr_db_->
                                getNeighbor (scan_info_->rsp->rec_bs_full[i].
                                             recommended_bs_id)->getDCD ()->
                                frequency * 1000);
                assert (ch != -1);
                WimaxRdvTimer *timer = new WimaxRdvTimer (this, ch);
                scan_info_->rdv_timers[scan_info_->nb_rdv_timers++] = timer;
                timer->start (getFrameDuration () *
                              scan_info_->rsp->rec_bs_full[i].rdv_time);
            }
        }
...
* msho = mobile subscriber handover request
*/
void
Mac802_16RS::send_msho_req ()
{
    debug2 ("RS %d send a handover request..\n", addr ());
    Packet *p;
    struct hdr_cmn *ch;
    hdr_mac802_16 *wimaxHdr;
    ...
    ch = HDR_CMN (p);
    wimaxHdr = HDR_MAC802_16 (p);
    p->allocdata (sizeof (struct mac802_16_mob_msho_req_frame) +
                  nbPref * sizeof (mac802_16_mob_msho_req_bs_index));
    ...
    req_frame->report_metric = 0x2;	//include RRSI
    ...
    debug2 ("RRSI=%e, %d, peer bs=%d\n", rssi,
            (u_char) ((rssi + 103.75) / 0.25), getPeerNode_head ()->getAddr ());
    ...
    {
        WimaxNeighborEntry *entry = nbr_db_->getNeighbors ()[i];
        //TBD: there is an error measuring RRSI for current BS during scanning
        //anyway, we don't put it in the least, so it's ok for now
        if (entry->isDetected ()
                && entry->getID () != getPeerNode_head ()->getAddr ())
        {
            req_frame->bs_full[j].neighbor_bs_index = entry->getID ();
            rssi =
                entry->getState ()->state_info->peer_list->lh_first->
                getStatWatch ()->average ();
            debug2 ("RRSI=%e, %d, neighbor bs=%d\n", rssi,
                    (u_char) ((rssi + 103.75) / 0.25),
                    req_frame->bs_full[j].neighbor_bs_index);
            req_frame->bs_full[j].bs_rssi_mean =
                (u_char) ((rssi + 103.75) / 0.25);
...
*/
void
Mac802_16RS::process_bsho_rsp (mac802_16_mob_bsho_rsp_frame * frame)
{
    ...
    while (head != NULL)
    {
        debug2 ("RS ARQ handling out cid %d\n", head->get_cid ());
        if ((head->getArqStatus () != NULL))
        {
            debug2 ("RS delete ARQ timer CID is [%d]\n", head->get_cid ());
...
*/
void
Mac802_16RS::process_nbr_adv (mac802_16_mob_nbr_adv_frame * frame)
{
    ...
    for (int i = 0; i < frame->n_neighbors; i++)
    {
        ...
        mac802_16_nbr_adv_info *info =
            (mac802_16_nbr_adv_info *) malloc (sizeof (mac802_16_nbr_adv_info));
        ...
        if (info->ucd_included)
        {
            ...
            for (int i = 0; i < nb_prof; i++)
            {
                debug2 ("\t Reading ul profile %i: f=%d, rate=%d, iuc=%d\n", i,
                        0, profiles[i].fec, profiles[i].uiuc);
            }
#endif
        }
        else
 ...
 */
void
Mac802_16RS::link_connect (int poa)
{
 ...
 */
bool
Mac802_16RS::set_mode (mih_operation_mode_t mode)
{
    switch (mode)
    {
    case NORMAL_MODE:
...
*/
void
Mac802_16RS::link_scan (void *req)
{
    if (!isScanRunning ())
    {
        setScanFlag (true);
...
*/
void
Mac802_16RS::update_watch (StatWatch * watch, double value)
{
    char *name;
    ...
    if (action != NO_ACTION_TH)
    {
        ...
        if (watch == &loss_watch_)
        {
            ...
            param.parameter_type = LINK_GEN_FRAME_LORS;
        }
...
*/
void
Mac802_16RS::update_throughput (ThroughputWatch * watch, double size)
{
    char *name;
    ...
    if (action != NO_ACTION_TH)
    {
        ...
        send_link_parameters_report (addr (), /*TODO revised function definiton not found:addr ()*/getPeerNode_head()->getAddr(), param, old_value,
                                     new_value);
    }
#else
    ...
    if (print_stats_)
        debug2 ("At %f in Mac %d, updating throughput stats %s: %f\n", NOW,
                addr (), name, watch->average ());
}

/**
* Start a new frame
*	called when dl timer expired
*/
void
Mac802_16RS::start_dlsubframe ()
{
    if (true)
    {
        ...
        debug
        ("At %f in Mac %d RS scheduler dlsubframe expires (frame=%d), r_ss :%lf, r_bs :%lf\n",
         NOW, addr (), frame_number_++, r_delta_ss, r_delta_bs);
        r_delta_bs = 0;

        assert (map_);
        map_->setStarttime (NOW);
        ((RSScheduler *) scheduler_)->schedule ();
        ...
        dl_timer_->resched (getFrameDuration ());
        ul_timer_->resched (map_->getUlSubframe ()->getStarttime () *
                            getPhy ()->getPS ());
        ul = 1;
    }
    else
...
Mac802_16RS::start_ulsubframe ()
{
    ...
    } else
    {
        ...
        if (getMap ()->getUlSubframe ()->getNbPdu () > 0)
        {
            ...
            getMap ()->getUlSubframe ()->getTimer ()->sched (b->getStarttime () *
                    getPhy ()->
                    getSymbolTime ());
...
Mac802_16RS::resume_scanning ()
{
    ...
    scan_info_->scn_timer_->start (scan_info_->rsp->scan_duration *
                                   getFrameDuration ());
...
*/
void
Mac802_16RS::setScanFlag (bool flag)
{
    scan_flag_ = flag;
}
...
void
Mac802_16RS::addPowerinfo (hdr_mac802_16 * wimaxHdr,
                           double power_per_subchannel, bool collision)
{

    //TODO:getNumSubcarrier UL OR DL
            ...
            if (wimaxHdr->phy_info.num_OFDMSymbol > num_symbol_per_slot)
            {
                ...
                for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                        ...
                        (wimaxHdr->phy_info.OFDMSymbol_offset +
                         num_symbol_per_slot)
                        /*wimaxHdr->phy_info.num_OFDMSymbol) */ ; i++)
                    for (int j = (wimaxHdr->phy_info.subchannel_offset);
                            j <= num_subchannel; j++)
                ...
                for (int i =
                        ...
                        wimaxHdr->phy_info.OFDMSymbol_offset +
                        wimaxHdr->phy_info.
                        num_OFDMSymbol /*wimaxHdr->phy_info.num_OFDMSymbol) */ ;
                        i++)
                    for (int j = 1;
                            j <=
                            ((wimaxHdr->phy_info.num_subchannels -
            ...
            else
            {
                ...
                for (int k = 0; k < wimaxHdr->phy_info.num_subchannels; k++)
                {
                    ...
                    assert (i <
                            ...
                            floor ((getPhy ()->getPS () *
                                    (int)
                                    floor ((getFrameDuration () /
                                            getPhy ()->getPS ()))) /
                                   getPhy ()->getSymbolTime ()));
...

        {
            ...
            if (wimaxHdr->phy_info.num_OFDMSymbol > 1)
            {
                for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                        i <
                        (wimaxHdr->phy_info.OFDMSymbol_offset +
                         1) /*wimaxHdr->phy_info.num_OFDMSymbol) */ ; i++)
                    for (int j = (wimaxHdr->phy_info.subchannel_offset);
                            j <= num_subchannel; j++)
                        (basepower_[i][j] = power_per_subchannel);
                // except the last num_symbol_per_slot and first num_symbol_per_slot whatever is thr
            ...
            else
            {
                ...
                for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                    ...
                    for (int j = (wimaxHdr->phy_info.subchannel_offset);
                            j <
                            wimaxHdr->phy_info.subchannel_offset +
                            wimaxHdr->phy_info.num_subchannels; j++)
                        (basepower_[i][j] = power_per_subchannel);
    ...
    else
    {
            ...
            //if(wimaxHdr->phy_info.num_OFDMSymbol >= num_symbol_per_slot)
        {
            ...
            else
            {
                ...
                for (int k = 0;
                        k <
                        wimaxHdr->phy_info.num_subchannels * num_data_subcarrier;
                        k++)
                {
...
bool
Mac802_16RS::IsCollision (const hdr_mac802_16 * wimaxHdr,
                          double power_subchannel)
{
//TODO: getNumsubchannels UL OR DL, Nothing seems to be different than that
    bool collision = FALSE;
    int num_subchannel = getPhy ()->getNumsubchannels (/*UL_*/DL_);
    int num_symbol_per_slot = getPhy ()->getSlotLength (/*UL_*/DL_);
        ...
        //if(wimaxHdr->phy_info.num_OFDMSymbol >= num_symbol_per_slot)
    {
        if (wimaxHdr->phy_info.num_OFDMSymbol > num_symbol_per_slot)
        {
            ...
            for (int i = wimaxHdr->phy_info.OFDMSymbol_offset;
                    i <
                    (wimaxHdr->phy_info.OFDMSymbol_offset +
                     num_symbol_per_slot) /*wimaxHdr->phy_info.num_OFDMSymbol) */ ;
                    i++)
            {
                for (int j = (wimaxHdr->phy_info.subchannel_offset);
                        j <= num_subchannel; j++)
                    if (basepower_[i][j] != power_subchannel)
                    {
                        collision = TRUE;
                        debug2
                        (" collision is true : i = %d j =%d basepower[i][j] = %.25f power_subchannel =%.25f \n",
            ...
            for (int i =
                    ...
                    i <
                    (wimaxHdr->phy_info.OFDMSymbol_offset) +
                    (wimaxHdr->phy_info.num_OFDMSymbol - num_symbol_per_slot); i++)
            {
                for (int j = 1; j <= num_subchannel; j++)
                    if (basepower_[i][j] != power_subchannel)
                    {
            ...
            for (int i =
                    ...
                    wimaxHdr->phy_info.OFDMSymbol_offset +
                    wimaxHdr->phy_info.
                    num_OFDMSymbol /*wimaxHdr->phy_info.num_OFDMSymbol) */ ; i++)
            {
                for (int j = 1;
                        j <=
                        ((wimaxHdr->phy_info.num_subchannels -
...
*/
void
Mac802_16RS::setCtrlAgent (WimaxCtrlAgent * agent)
{
    assert (agent);
    ctrlagent_ = agent;
}
...
*/
void
Mac802_16RS::process_ranging_req (Packet * p)
{
    UlSubFrame *ulsubframe = getMap ()->getUlSubframe ();
    mac802_16_rng_req_frame *req = (mac802_16_rng_req_frame *) p->accessdata ();

    debug ("RS received ranging request from :%d\n",
           HDR_MAC802_16 (p)->header.cid);
    ...
    {
        //here we can make decision to accept the RS or not.
        //for now, accept everybody
        //check if CID already assigned for the RS
        ...
        if (peer == NULL)
        {
            ...
            Connection *upbasic =
                new Connection (CONN_BASIC, basic->get_cid ());
            Connection *primary = new Connection (CONN_PRIMARY);
            Connection *upprimary =
                new Connection (CONN_PRIMARY, primary->get_cid ());
            ...
            mac802_16_rng_rsp_frame *frame =
                (mac802_16_rng_rsp_frame *) rep->accessdata ();
            ...
            //enqueue packet
            getCManager ()->get_connection (BROADCAST_CID,
                                            OUT_CONNECTION)->enqueue (rep);
            debug
            ("RS created ranging response MAC_RNG_RSP to RSID :%d, CID :%d, primary :%d, size :%d\n",
             req->ss_mac_address, HDR_MAC802_16 (p)->header.cid,
             primary->get_cid (), ch->size ());
            ...
            {
                cl_tail_->next =
                    (new_client_t *) malloc (sizeof (new_client_t));
        ...
        {
            debug
            ("RS received ranging for known station (%d) and create MAC_RNG_RSP\n",
             req->ss_mac_address);
            //reset invited ranging retries for RS
            //create packet for answers
...
*/
void
Mac802_16RS::process_bw_req (Packet * p)
{
    hdr_mac802_16 *wimaxHdr = HDR_MAC802_16 (p);
    gen_mac_header_t header = wimaxHdr->header;
    ...
    if ((req->type & 0x7) == 0x001)
    {
        ...
        debug10 ("RS (setBW) Aggregate request on connection %d of %d\n",
                 c->get_cid (), c->getBw ());
    }
    else if ((req->type & 0x7) == 0x000)
    {
        ...
        debug10 ("RS (setBW) Incremental request on connection %d of %d\n",
                 c->get_cid (), c->getBw ());
...
*/
void
Mac802_16RS::process_cdma_req (Packet * p)
{
    hdr_mac802_16 *wimaxHdr = HDR_MAC802_16 (p);
    gen_mac_header_t header = wimaxHdr->header;
    ...
    debug
    ("RS (process_cdma_req: Impossible) received cdma request code :%d from :%d\n",
     req->code, req->cid);

    //retrieve the CID and update bandwidth request information
    Connection *c = getCManager ()->get_connection (req->cid, IN_CONNECTION);
    assert (c);
    debug10 ("RS (setCDMA), code :%d, top :%d, ssid :%d\n", req->code, req->top,
             req->br);
...
Mac802_16RS::process_reg_req (Packet * req)
{
    ...
    peer =
        getCManager ()->get_connection (header_req.cid,
                                        IN_CONNECTION)->getPeerNode ();
    ...
    if (secondary == NULL)
    {
        ...
        Connection *upsecondary =
            new Connection (CONN_SECONDARY, secondary->get_cid ());
    ...
    debug
    ("RS created registration response MAC_REG_RSP to :%d, secondary :%d, size :%d\n",
...
*/
void
Mac802_16RS::send_nbr_adv ()
{
    debug ("At %f in RS %d send_nbr_adv (nb_neighbor=%d)\n", NOW, addr (),
           nbr_db_->getNbNeighbor ());
    Packet *p;
    struct hdr_cmn *ch;
    hdr_mac802_16 *wimaxHdr;
    ...
    for (int i = 0; i < frame->n_neighbors; i++)
    {
        ...
        memcpy (&(frame->nbr_info[i].dcd_settings),
                nbr_db_->getNeighbors ()[i]->getDCD (),
                sizeof (mac802_16_dcd_frame));
        frame->nbr_info[i].ucd_included = true;
        memcpy (&(frame->nbr_info[i].ucd_settings),
                nbr_db_->getNeighbors ()[i]->getUCD (),
                sizeof (mac802_16_ucd_frame));
...
*/
void
Mac802_16RS::process_msho_req (Packet * req)
{
    hdr_mac802_16 *wimaxHdr_req = HDR_MAC802_16 (req);
    gen_mac_header_t header_req = wimaxHdr_req->header;
    mac802_16_mob_msho_req_frame *req_frame =
    ...
    //check the RS that has stronger power
    int maxIndex = 0;
    int maxRssi = 0;		//max value
    debug2 ("In the MSHO Request message, there are %d new RS full entities.\n",
    ...
    ch = HDR_CMN (p);
    wimaxHdr = HDR_MAC802_16 (p);
    p->allocdata (sizeof (struct mac802_16_mob_bsho_rsp_frame) +
                  sizeof (mac802_16_mob_bsho_rsp_rec));
...
*/
void
Mac802_16RS::process_ho_ind (Packet * p)
{
    ...
    debug ("At %f in Mac %d received handover indication from %d\n", NOW,
           addr (), header_req.cid);
    debug2 ("xingting is trying to remove all the connections to RS.\n");
    ...
    while (head != NULL)
    {
        ...
        if ((head->getArqStatus () != NULL))
        {
            debug2 ("RS %d delete ARQ timer CID is [%d]\n", addr (),
                    head->get_cid ());
            head->getArqStatus ()->cancelTimer ();
            //delete head->getArqStatus();
            //getCManager()->remove_connection (head);
...
*/
void
Mac802_16RS::send_scan_response (mac802_16_mob_scn_rsp_frame * rsp, int cid)
{
    //create packet for request
    Packet *p = getPacket ();
    struct hdr_cmn *ch = HDR_CMN (p);
    ...
    mac802_16_mob_scn_rsp_frame *rsp_frame =
        (mac802_16_mob_scn_rsp_frame *) p->accessdata ();
    ...
    PeerNode *peer =
        getCManager ()->get_connection (cid, false)->getPeerNode ();
    ...
    */
    ScanningStation *sta =
        new ScanningStation (peer->getAddr (), rsp_frame->scan_duration & 0xFF,
...
*/
void
Mac802_16RS::addNewFastRanging (double time, int macAddr)
{
...
*/
void
Mac802_16RS::addtimer17 (int index)
{
...
*/
void
Mac802_16RS::removetimer17 (int index)
{
    //clean expired timers
    T17Element *entry;
...
*/
bool
Mac802_16RS::isPeerScanning (int nodeid)
{
    ScanningStation *sta;
...
*/
void
Mac802_16RS::process_mac_packet (Connection * con, Packet * p)
{
    //debug2 ("Mac802_16RS received packet to process\n");

    assert (HDR_CMN (p)->ptype () == PT_MAC);
    debug10 ("Mac802_16RS received packet to process\n");

    hdr_mac802_16 *wimaxHdr = HDR_MAC802_16 (p);
    gen_mac_header_t header = wimaxHdr->header;

    //TODO: bandwidth request related codes here
    bw_req_header_t *req;
    req = (bw_req_header_t *) & header;
    ...
    */
    ...
    if ((mac802_16_dl_map_frame *) p->accessdata () == NULL)
    {
        if (header.type_arqfb == 1)
        {
            ...
            {
                Connection *connection =
                    this->getCManager ()->get_connection (wimaxHdr->arq_ie[i].cid,
                                                          true);
                if (connection)
                {
                    if (connection->getArqStatus ()
                    ...
                    {
                        debug2
                        ("ARQ:RS Feedback Payload Received: Has a feedback: Value of i:%d , Value of number of acks:%d \n",
                         i, wimaxHdr->num_of_acks);
                        connection->getArqStatus ()->arqRecvFeedback (p, i,
                                connection);
                    }
                }
            }
    ...
    case MAC_DL_MAP:
    {
        ...
        debug2 ("RS MAC_DL_MAP At %f frame start at %f\n", NOW,
                getMap ()->getStarttime ());
        ...
        {
            peer->getStatWatch ()->update (10 *
                                           log10 (p->txinfo_.RxPr * 1e3));
            debug
            ("At %f in Mac %d weighted RXThresh: %e rxp average %e lgd %d\n",
             NOW, index_, macmib_.lgd_factor_ * macmib_.RXThreshold_,
             pow (10, peer->getStatWatch ()->average () / 10) / 1e3);
            double avg_w =
                pow (10, (peer->getStatWatch ()->average () / 10)) / 1e3;
            ...
            if (avg_w < (macmib_.lgd_factor_ * macmib_.RXThreshold_))
            {
                if (state_ == MAC802_16_CONNECTED)
                {
                    ...
                    {
                        double probability =
                            ((macmib_.lgd_factor_ * macmib_.RXThreshold_) -
                             avg_w) / ((macmib_.lgd_factor_ *
                                        macmib_.RXThreshold_) -
                                       macmib_.RXThreshold_);
                        Mac::send_link_going_down (addr (), peer->getAddr (),
                                                   -1,
                                                   (int) (100 * probability),
                                                   LGD_RC_LINK_PARAM_DEGRADING,
                                                   eventId_++);
                    }
                    else
                    {
            ...
            else
            {
                ...
                {
#ifdef USE_802_21
                    Mac::send_link_rollback (addr (), peer->getAddr (),
                                             eventId_ - 1);
#endif
    ...
    case MAC_MOB_SCN_REQ:
        if (ctrlagent_)
            ...
            fprintf (stderr,
                     "Warning: no controler to handle scan request in RS %d\n",
                     addr ());
        break;
