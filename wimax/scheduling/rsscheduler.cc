...
/**
 * Bridge to TCL for RSScheduler
 */
int frame_num=0;

//This structure is used for virtual allocation and "index_burst2" is used for #counting bursts
int index_burst2=0;
struct virtual_burst
{
    ...
    int dl_ul;    //0 = DL, 1 = UL
    int ie_type;
} virtual_alloc2[MAX_MAP_IE], ul_virtual_alloc2[MAX_MAP_IE];


static class RSSchedulerClass : public TclClass
{
public:
    RSSchedulerClass() : TclClass("WimaxScheduler/RS") {}
    TclObject* create(int, const char*const*)
    {
        return (new RSScheduler());
 ...
 * Create a scheduler
 */
RSScheduler::RSScheduler () : WimaxScheduler ()
{
    debug2 ("RSScheduler created\n");
...
/**
 * Return the MAC casted to RSScheduler
 * @return The MAC casted to RSScheduler
 */
Mac802_16RS* RSScheduler::getMac()
{
    return (Mac802_16RS*)mac_;
}
 ...
 */
int RSScheduler::command(int argc, const char*const* argv)
{
    if (argc == 3)
    {
 ...
 */
int RSScheduler::getBWopportunity ()
{
 ...
 */
int RSScheduler::getInitRangingopportunity ()
{
...
int RSScheduler::increase_dl_map_ie(int num_of_entries, int totalslots, int num_ie)
{
...
//padding to byte based
    float dl_map_increase_byte = virtual_alloc2[0].byte + num_ie*dl_ie_size;
    int dl_map_increase_slots = virtual_alloc2[0].rep*(int)ceil(ceil(dl_map_increase_byte)/(double)phy->getSlotCapacity(virtual_alloc2[0].mod,DL_));

    debug10 ("Check_DLMAP.From array DL MAP byte :%f, slots :%d, IE_SIZE :%f, slotcapacity :%d, #ie :%d\n", virtual_alloc2[0].byte, virtual_alloc2[0].numslots, dl_ie_size, phy->getSlotCapacity(virtual_alloc2[0].mod,DL_), num_ie);
    debug10 ("\tNew DL MAP byte :%f, slots :%d\n", dl_map_increase_byte, dl_map_increase_slots);

    for (int i = 1; i<num_of_entries; i++)
    {
        debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_alloc2[i].numslots, slots_sofar);
        slots_sofar = slots_sofar + virtual_alloc2[i].numslots;
    }
    ...
    {
        debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, #ie :%d\n", virtual_alloc2[0].byte, virtual_alloc2[0].numslots, dl_map_increase_byte, dl_map_increase_slots, num_ie);

        virtual_alloc2[0].byte = dl_map_increase_byte;
        virtual_alloc2[0].numslots = dl_map_increase_slots;
        return slots_sofar;
    }
    else
    {
...
int RSScheduler::overallocation_withdlmap(int num_of_entries, int totalslots, int ownslots)
{
    ...
    float dl_ie_size = DL_MAP_IE_SIZE;
//padding to byte based
    float dl_map_increase_byte = virtual_alloc2[0].byte + dl_ie_size;
    int dl_map_increase_slots = virtual_alloc2[0].rep*(int)ceil((ceil)(dl_map_increase_byte)/(double)phy->getSlotCapacity(virtual_alloc2[0].mod,DL_));

    debug10 ("Check_DLMAP.From array DL MAP byte :%f, slots :%d, IE_SIZE :%f, slotcapacity :%d\n", virtual_alloc2[0].byte, virtual_alloc2[0].numslots, dl_ie_size, phy->getSlotCapacity(virtual_alloc2[0].mod,DL_));
    debug10 ("\tNew DL MAP byte :%f, slots :%d\n", dl_map_increase_byte, dl_map_increase_slots);
    ...
    if ( slots_sofar <= totalslots)
    {
        debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, available burst slot size :%d\n", virtual_alloc2[0].byte, virtual_alloc2[0].numslots, dl_map_increase_byte, dl_map_increase_slots, ownslots);
    ...
    else
    {
        ...
        {
            debug10 ("Check_DLMAP. Old DL map byte :%f, slots :%d, New DL map byte :%f, slots :%d, available slot burst size :%d\n", virtual_alloc2[0].byte, virtual_alloc2[0].numslots, dl_map_increase_byte, dl_map_increase_slots, totalslots - slots_temp - dl_map_increase_slots);
            virtual_alloc2[0].byte = dl_map_increase_byte;
            virtual_alloc2[0].numslots = dl_map_increase_slots;
...
int RSScheduler::max_conn_withdlmap(int num_of_entries, int totalslots)
{
    ...
    float virtual_increase_byte = virtual_alloc2[0].byte;
    int virtual_increase_slots = virtual_alloc2[0].numslots;
...
//	debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_alloc2[i].numslots, slots_sofar);
    ...
    int slots_data = slots_sofar - virtual_alloc2[0].numslots;

    debug10 ("Check_DLMAP. Max supported conn total_dl_slots :%d, sofar slots :%d, dlmap slots :%d, dlmap byte :%f, available slots :%d\n", totalslots, slots_sofar, virtual_alloc2[0].numslots, virtual_alloc2[0].byte, totalslots-slots_sofar);

    while (1)
    {
        virtual_increase_byte += dl_ie_size;
        int virtual_increase_byte_ceil = (int)ceil(virtual_increase_byte);
        virtual_increase_slots = virtual_alloc2[0].rep*ceil((double)(virtual_increase_byte_ceil)/(double)phy->getSlotCapacity(virtual_alloc2[0].mod,DL_));
...
int RSScheduler::freeslots_withdlmap_given_conn(int num_of_entries, int totalslots, int newconn)
{
...
//	debug2 ("\t index :%d, numslots :%d, accum :%d\n", i, virtual_alloc2[i].numslots, slots_sofar);
    ...
    debug10 ("Check_DLMAP. #Free slots newconn :%d, sofar slots :%d, dlmap slots :%d, dlmap byte :%f, newdlmap slots :%d, newdlmap byte :%d\n", newconn, slots_sofar, virtual_alloc2[0].numslots, virtual_alloc2[0].byte, virtual_increase_slots, virtual_increase_byte_ceil);
...
int RSScheduler::overallocation_withoutdlmap(int num_of_entries, int totalslots, int ownslots)
{
...
void RSScheduler::bubble_sort (int arrayLength, con_data_alloc array[], int sort_field)
{
    int i, j, flag = 1;
    con_data_alloc temp;
...
int RSScheduler::doesvirtual_allocexist(int num_of_entries, int cid)
{
    for (int i = 0; i<num_of_entries-1; i++)
    {
        if (virtual_alloc2[i].cid == cid)
        {
            return cid;
        }
    }
...
int RSScheduler::addslots_withoutdlmap(int num_of_entries, int byte, int slots, int cid)
{
    for (int i = 0; i<num_of_entries-1; i++)
    {
        if (virtual_alloc2[i].cid == cid)
        {
            virtual_alloc2[i].byte += byte;
            virtual_alloc2[i].numslots += slots;
            return i;
        }
    }
    return -1;
...
int RSScheduler::check_overallocation(int num_of_entries)
{
    ...
    if (true) {
                ...
                n; n=n->next_entry())
        {
            if (n->getArqStatus () != NULL && n->getArqStatus ()->isArqEnabled() == 1)
            {
                ...
                {
                    peernode = n->getPeerNode ();
                    //TODO:scheduler diff 1 arqfb_in_dl_data
                    ...
                    if (out_datacnx_exists == false)
                    {
                        //debug2("ARQ RS: Feedback in Basic Cid \n");
                        basic = peernode->getBasic (OUT_CONNECTION);
                        pfb = mac_->getPacket ();
                        wimaxHdrMap= HDR_MAC802_16(pfb);
                        wimaxHdrMap->header.cid = basic->get_cid ();
                    ...
                    {
                        debug2("ARQ RS : Feedback in data Cid \n");
                        OutData = peernode->getOutData ();
                        pfb = OutData->dequeue ();
                        wimaxHdrMap= HDR_MAC802_16(pfb);
                        if (wimaxHdrMap->header.type_arqfb == 1)
                        {
                            debug2("ARQ RS: Feedback already present, do nothing \n");
                            OutData->enqueue_head (pfb);
                            continue;
                        }
                        wimaxHdrMap->num_of_acks = 0;
                    ...
                    wimaxHdrMap->num_of_acks = wimaxHdr->num_of_acks;
                    HDR_CMN(pfb)->size()	+= (wimaxHdr->num_of_acks * HDR_MAC802_16_ARQFEEDBK_SIZE);
                    debug2("ARQ : In RSScheduler: Enqueueing an feedback cid: %d arq_ie->fsn:%d \n", wimaxHdr->arq_ie[0].cid,  wimaxHdr->arq_ie[0].fsn);
                    if (out_datacnx_exists == false)
                    {
                        // If I am here then the Ack packet has been created, so we will enqueue it in the Basic Cid
                        basic->enqueue (pfb);
...
//| | | |       => symbols
//0 1 2 3       => dlduration
        debug10 ("Start RS Scheduling: TotalSymbols :%d, MAXDL :%d, MAXUL :%d, Preamble :%d, dlduration after preamble :%d\n", nbSymbols, maxdlduration, maxulduration, DL_PREAMBLE, dlduration);
        ...
        // This cdma_flag is used in case there is no peer yet however the allocation for cdma_transmission oppportunity needed to be allocated by the RS-UL scheduler
        int cdma_flag = 0;
        Connection *con1;
...
// From UL_MAP_IE, we map the allocation into UL_BURST
        ...
        bzero(virtual_alloc2, MAX_MAP_IE*sizeof(virtual_burst));
        index_burst2 = 0;

//1. Virtual DL_MAP
//Note that, we virtually allocate all burst into "virtual_alloc2[]", then we will physically allocate/map those allocation into burst/physical later. The reason is because we have variable part of DL_MAPs
//"index_burst2" is the #total_burst including each DL_MAP, UL_MAP, DCD, UCD, other broadcast message and each CID allocation
//In this version, the allocation is per CID/burst for downlink
        ...
        virtual_alloc2[index_burst2].alloc_type = 0;
        virtual_alloc2[index_burst2].cid = BROADCAST_CID;
        virtual_alloc2[index_burst2].n_cid = 0;
        virtual_alloc2[index_burst2].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
        virtual_alloc2[index_burst2].preamble = true;
        virtual_alloc2[index_burst2].numslots = fixed_dl_map_slots;
        virtual_alloc2[index_burst2].mod = fixed_dl_map_mod;
        virtual_alloc2[index_burst2].byte = fixed_byte_dl_map;
        virtual_alloc2[index_burst2].rep =  Repetition_code_;
        virtual_alloc2[index_burst2].dl_ul = 0;
        virtual_alloc2[index_burst2].ie_type = 0;
        index_burst2++;
...
//2. Virtual UL_MAP:
        ...
        virtual_alloc2[index_burst2].alloc_type = 1;
        virtual_alloc2[index_burst2].cid = BROADCAST_CID;
        virtual_alloc2[index_burst2].n_cid = 0;
        virtual_alloc2[index_burst2].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
        virtual_alloc2[index_burst2].preamble = true;
        virtual_alloc2[index_burst2].numslots = fixed_ul_map_slots;
        virtual_alloc2[index_burst2].mod = fixed_ul_map_mod;
        virtual_alloc2[index_burst2].byte = fixed_byte_ul_map;
        virtual_alloc2[index_burst2].rep =  Repetition_code_;
        virtual_alloc2[index_burst2].dl_ul = 0;
        virtual_alloc2[index_burst2].ie_type = 0;
        int add_ie_to_dlmap = increase_dl_map_ie(index_burst2, total_dl_slots_pusc, 1);
...
//3. Virtual DCD
        if (getMac()->sendDCD || map->getDlSubframe()->getCCC()!= getMac()->dlccc_)
        {
            ...
            virtual_alloc2[index_burst2].alloc_type = 2;
            virtual_alloc2[index_burst2].cid = BROADCAST_CID;
            virtual_alloc2[index_burst2].n_cid = 1;
            virtual_alloc2[index_burst2].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
            virtual_alloc2[index_burst2].preamble = false;
            virtual_alloc2[index_burst2].numslots = dcd_slots;
            virtual_alloc2[index_burst2].mod = dcd_mod;
            virtual_alloc2[index_burst2].byte = byte_dcd;
            virtual_alloc2[index_burst2].rep =  rep_dcd;
            virtual_alloc2[index_burst2].dl_ul = 0;
            virtual_alloc2[index_burst2].ie_type = 0;
            int add_ie_to_dlmap = increase_dl_map_ie(index_burst2, total_dl_slots_pusc, 1);
            if (add_ie_to_dlmap <0 )
...
//4. Virtual UCD
        if (getMac()->sendUCD || map->getUlSubframe()->getCCC()!= getMac()->ulccc_)
        {
            ...
            virtual_alloc2[index_burst2].alloc_type = 3;
            virtual_alloc2[index_burst2].cid = BROADCAST_CID;
            virtual_alloc2[index_burst2].n_cid = 1;
            virtual_alloc2[index_burst2].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
            virtual_alloc2[index_burst2].preamble = false;
            virtual_alloc2[index_burst2].numslots = ucd_slots;
            virtual_alloc2[index_burst2].mod = ucd_mod;
            virtual_alloc2[index_burst2].byte = byte_ucd;
            virtual_alloc2[index_burst2].rep =  rep_ucd;
            virtual_alloc2[index_burst2].dl_ul = 0;
            virtual_alloc2[index_burst2].ie_type = 0;
            int add_ie_to_dlmap = increase_dl_map_ie(index_burst2, total_dl_slots_pusc, 1);
            if (add_ie_to_dlmap <0 )
        ...
        int check_dl_slots = check_overallocation(index_burst2);

        debug10 ("DL.broadcast messages (slots): (v)Fixed_DL_MAP :%d, (v)UL_MAP :%d, DCD :%d, UCD :%d, MaxDL :%d\n", virtual_alloc2[0].numslots, virtual_alloc2[1].numslots, virtual_alloc2[2].numslots, virtual_alloc2[3].numslots, total_dl_slots_pusc);
        debug10 ("\t(bytes): (v)Fixed_DL_MAP :%f, (v)UL_MAP :%f, DCD :%f, UCD :%f\n", virtual_alloc2[0].byte, virtual_alloc2[1].byte, virtual_alloc2[2].byte, virtual_alloc2[3].byte);
        ...
        debug2("%d\n",mac_->getCManager()->get_connection(BROADCAST_CID,OUT_CONNECTION)/*->queueByteLength()*/);
        debug10("1011111\n");
...
//5. Virtual Other broadcast
        if (mac_->getCManager()->get_connection (BROADCAST_CID, OUT_CONNECTION)->queueByteLength()>0)
        {
            ...
            virtual_alloc2[index_burst2].alloc_type = 4;
            virtual_alloc2[index_burst2].cid = BROADCAST_CID;
            virtual_alloc2[index_burst2].n_cid = 1;
            virtual_alloc2[index_burst2].iuc = map->getDlSubframe()->getProfile (DIUC_PROFILE_2)->getIUC();
            virtual_alloc2[index_burst2].preamble = false;
            virtual_alloc2[index_burst2].numslots = bc_slots;
            virtual_alloc2[index_burst2].byte = real_bytes;
            virtual_alloc2[index_burst2].rep =  1;
            virtual_alloc2[index_burst2].dl_ul = 0;
            virtual_alloc2[index_burst2].ie_type = 0;
            int add_ie_to_dlmap = increase_dl_map_ie(index_burst2, total_dl_slots_pusc, 1);
            if (add_ie_to_dlmap <0 )
            ...
            debug10 ("DL.other_broadcast messages (slots): %d, (bytes): %f\n", virtual_alloc2[index_burst2].numslots, virtual_alloc2[index_burst2].byte);
            index_burst2++;
        }
//End of Virtual allocation for all broadcast message
        debug10("10111\n");
        int index_burst2_before_data = index_burst2;
        ...
        //Add virtual allocation to physical DL burst
          ...
          for (int i=0; i<index_burst2; i++) {
              db = (DlBurst*) map->getDlSubframe()->getPdu ()->addBurst (nbdlbursts++);
              db->setCid (virtual_alloc2[i].cid);
              db->setIUC (virtual_alloc2[i].iuc);
              db->setPreamble(virtual_alloc2[i].preamble);
              db->setStarttime(virtual_symbol_offset);
              db->setSubchannelOffset(virtual_subchannel_offset);

              int num_of_subchannel = virtual_alloc2[i].numslots;
        ...
        */
        ...
        int check_dl_duration = ceil((double)check_overallocation(index_burst2)/(double)total_dl_subchannel_pusc);
...
//    b_data += ch->size();
        ...
        debug2("RS scheduler is going to handle other bursts (not DL/UL_MAP, DCD/UCD), #bursts = %d\n", map->getDlSubframe()->getPdu ()->getNbBurst());
        for (int index = num_burst_before_data ; index < map->getDlSubframe()->getPdu ()->getNbBurst(); index++)
        {
            ...
            if (c!=NULL)
            {
                ...
                if (c->isFragEnable() && c->isPackingEnable() &&  (c->getArqStatus () != NULL) && (c->getArqStatus ()->isArqEnabled() == 1))
                {
                    debug2("DL/UL.RSSscheduler is goting to transfer packet with fragackarq.\n");
...
/*       // rpi removed this dl burst ----------------------------------------------------------------------------
	 int RSScheduler::addDlBurst (int burstid, Connection *c, int iuc, int dlduration, int maxdlduration)
	 {
	 double txtime; //tx time for some data (in second)
	 int txtime_s;  //number of symbols used to transmit the data
 ...
 * @param the new allocation status
 */
void RSScheduler::addDlBurst (int burstid, int cid, int iuc, int ofdmsymboloffset, int numofdmsymbols, int subchanneloffset, int numsubchannels)
{
    DlBurst *db = (DlBurst*) mac_->getMap()->getDlSubframe()->getPdu ()->addBurst (burstid);
    db->setCid (cid);
    db->setIUC (iuc);
...
int RSScheduler::doesMapExist(int cid, int *cid_list, int num_of_entries)
{
    for (int i = 0; i<num_of_entries; ++i)
    {
        if (cid_list[i]==cid)
...
mac802_16_dl_map_frame * RSScheduler::dl_stage2(Connection *head, int input_subchannel_offset, int total_subchannels, int total_symbols, int symbol_start, int stripping, int total_dl_slots_pusc)
{
    ...
    int temp_index_burst2 = index_burst2;
        ...
        while (con!=NULL)
        {
            if (con->get_category() == contype)
            {
                ...
                if ( con_byte > 0 )
                {
                    ...
                    int virtual_alloc2_exist = doesvirtual_allocexist(index_burst2, con->get_cid());
                    int add_slots = 0;
                    if (virtual_alloc2_exist > 0)
                    {
                        int add_slots = overallocation_withoutdlmap (index_burst2, total_dl_slots_pusc, con_slots);
                        int t_index = addslots_withoutdlmap(index_burst2, con_byte, con_slots, con->get_cid());
                        debug10 ("DL Add more slots into existing burst contype(%d), index :%d, cid :%d, byte :%d, slots :%f\n", contype, t_index, con->get_cid(), virtual_alloc2[t_index].byte, virtual_alloc2[t_index].numslots);
                    }
                    else
                    {
                        int add_slots = overallocation_withdlmap (index_burst2, total_dl_slots_pusc, con_slots);
                        if (add_slots > 0)
                        {
                            virtual_alloc2[index_burst2].alloc_type = 5;
                            virtual_alloc2[index_burst2].cid = con->get_cid();
                            virtual_alloc2[index_burst2].n_cid = 1;
                            //virtual_alloc2[index_burst2].iuc = getDIUCProfile(con_mod);
                            virtual_alloc2[index_burst2].iuc = getDIUCProfile(OFDM_QPSK_1_2);
                            virtual_alloc2[index_burst2].numslots = add_slots;
                            virtual_alloc2[index_burst2].byte = (add_slots * mac_->getPhy()->getSlotCapacity(con_mod, DL_));
                            virtual_alloc2[index_burst2].rep =  rep;
                            virtual_alloc2[index_burst2].dl_ul = 0;
                            virtual_alloc2[index_burst2].ie_type = 0;
                            debug10 ("DL Add new burst contype(%d), index :%d, cid :%d, byte :%f, slots :%f\n", contype, index_burst2, con->get_cid(), virtual_alloc2[index_burst2].byte, virtual_alloc2[index_burst2].numslots);
                            index_burst2++;
                        }
                    }
                }//end if q > 0

                debug10 ("DL2.management msg, contype(%d), index :%d, CID :%d, Q-Bytes :%d, allocationsize :%f\n", contype, index_burst2, con->get_cid(), con->queueByteLength(), con_byte);
            }
            con = con->next_entry();
        }//end con!=null
    }//end for loop
...
//End ARQ aware scheduling
                    ...
                    {
                        int add_slots = overallocation_withoutdlmap (index_burst2, total_dl_slots_pusc, con_slots);
                        addslots_withoutdlmap(index_burst2, con_byte, con_slots, con->get_cid());
                    }
                    else
                    {
                        int add_slots = overallocation_withdlmap (index_burst2, total_dl_slots_pusc, con_slots);
...
//Find out how many connections can be supported given variable DL MAP
    int max_conn_dlmap = max_conn_withdlmap(index_burst2, total_dl_slots_pusc);
...
//Sorting and make sure #request slots is correct
    ...
    if (j_need > 0 ) t_freeslots = freeslots_withdlmap_given_conn(index_burst2, total_dl_slots_pusc, j_need);
...
//Assign granted slots to each connection (virtual burst)
    for (int i=0; i<j_need; i++)
    {
        if (con_data[i].grant_slots >=0 )
        {
            ...
            int virtual_alloc2_exist = doesvirtual_allocexist(index_burst2, con_data[i].cid);
            int add_slots = 0;
            if (virtual_alloc2_exist > 0)
            {
                int add_slots = overallocation_withoutdlmap (index_burst2, total_dl_slots_pusc, con_slots);
                addslots_withoutdlmap(index_burst2, con_byte, con_slots, con_data[i].cid);
            }
            else
            {
                int add_slots = overallocation_withdlmap (index_burst2, total_dl_slots_pusc, con_slots);
                ...
                {
                    virtual_alloc2[index_burst2].alloc_type = 5;
                    virtual_alloc2[index_burst2].cid = con_data[i].cid;
                    virtual_alloc2[index_burst2].n_cid = 1;
                    virtual_alloc2[index_burst2].iuc = getDIUCProfile(con_mod);
                    virtual_alloc2[index_burst2].numslots = add_slots;
                    virtual_alloc2[index_burst2].byte = (add_slots * mac_->getPhy()->getSlotCapacity(con_mod, DL_));
    ...
    */
    ...
    int total_slots_sofar = check_overallocation(temp_index_burst2);
    ...
    // In this version, we do the stripping outside the dl_stage2
      if(stripping == VERTICAL_STRIPPING){
        for(i=0; i<index_burst2-temp_index_burst2; ++i){
          ies[i].cid = virtual_alloc2[i+temp_index_burst2].cid;
          num_of_slots = virtual_alloc2[i+temp_index_burst2].numslots;
          ies[i].diuc = virtual_alloc2[i+temp_index_burst2].iuc;
    ...
    */
    ...
    ie_index = index_burst2 - temp_index_burst2;
...
//mac802_16_dl_map_frame * RSScheduler::dl_stage2(Connection *head, int total_subchannels, int total_symbols, int symbol_start, int  stripping){
mac802_16_dl_map_frame * RSScheduler::dl_stage2(Connection *head, int input_subchannel_offset,  int total_subchannels, int total_symbols, int symbol_start, int  stripping){
  struct mac802_16_dl_map_frame *dl_map;
  struct mac802_16_dlmap_ie *ies;
  Connection *con;
  int i, ie_index, temp_index;
 ...
 * Added by Ritun
 */
diuc_t RSScheduler::getDIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
/* Added by Ritun
 */
uiuc_t RSScheduler::getUIUCProfile(Ofdm_mod_rate rate)
{
    Profile *p;
...
struct mac802_16_ul_map_frame * RSScheduler::ul_stage2(Connection *head, int total_subchannels, int total_symbols, int symbol_start, int stripping)
{
 ...
 */
void RSScheduler::create_cdma_request (Connection *con)
{
